...
class TabForm(Form):
	...
	def __init__(self, tableData, elements, parameterName, inUserSelectedStrIds):
		...
		#	print("elements: Id {0} Name {1} Type {2}, Category {3}".format(el.Id, el.Name, el.GetType().Name, el.Category.Name))
		...
		try:
			...
			print("self.InitializeComponent() in TabForm.__init__() error: {0}".format(ex))
			raise ValueError("self.InitializeComponent() in TabForm.__init__() error:", ex)
	...
	def InitializeComponent(self):
		...
		try:
			...
			print("self.setupDataGridView() in TabForm.InitializeComponent() error: {0}".format(ex))
			raise ValueError("self.setupDataGridView() in TabForm.InitializeComponent() error:", ex)	
	...
	def setupDataGridView(self):
		...
		try:
			...
			print("self.values in TabForm.setupDataGridView() error: {0}".format(ex))
			raise ValueError("self.values in TabForm.setupDataGridView() error:", ex)
		...
		try:
			...
			print("self.getDataSources() in TabForm.setupDataGridView() error: {0}".format(ex))
			raise ValueError("self.getDataSources() in TabForm.setupDataGridView() error:", ex)
	...
	def configureButtons(self):
		...
		self.isolateButton.Width = self.Width / 2
	...
	def getDataSources(self, inTableData, **kwargs):
		...
		try:
			for i,v in enumerate(inTableData):
				elId = v.Id.ToString()
				elCategory = "{}".format(v.Category.Name)
				parameterValue = "{}".format(self.values[i] if len(self.values) > i else "Nd")
				markValue = "{}".format(self.markValues[i] if len(self.markValues) > i else "Nd")
				typeMarkValue = "{}".format(self.typeMarkValues[i] if len(self.typeMarkValues) > i else "Nd")
				elName = "{}".format(v.Name if hasattr(v, "Name") else v.FamilyName)
				dic = {self.columnNames[0] : elId, \
												self.columnNames[1] : elCategory, \
												self.columnNames[2] : elName, \
			...
			print("enumerate(inTableData) in TabForm.getDataSources() error: {0}".format(ex))
			raise ValueError("enumerate(inTableData) in TabForm.getDataSources() error:", ex)
	...
	def selectionChanged(self, sender, event):
		...
		if len(uniqueValues) == 1:
			...
			self.setParameterTextBox.Text = "**multiple values selected**"
...
class MainForm(Form):
	...
	def update(self, sender, event):
		...
		try:
			...
			print("self.filteredElements in MainForm.update() error: {0}".format(ex))
			raise ValueError("self.filteredElements in MainForm.update() error", ex)
		...
		try:
			...
			print("getValuesByParameterName in MainForm.update() error: {0}".format(ex))
			raise ValueError("getValuesByParameterName in MainForm.update() error", ex)
		try:
			...
			print("self.elementTab = TabForm	 in MainForm.update() error: {0}".format(ex))
			raise ValueError("self.elementTab = TabForm	 in MainForm.update() error", ex)
		...
		for el in inElements:
			if el.GetTypeId().IntegerValue > -1:
				...
				if el.LookupParameter(inParameterName):
					...
					filteredElements.append(el)
		filtereElementsIds = [x.Id for x in filteredElements]
		filteredElementsIdsCol = Clist[ElementId](filtereElementsIds)
		viewsIds = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Views).	WhereElementIsNotElementType().ToElementIds()
		camerasIds = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Cameras).	WhereElementIsNotElementType().ToElementIds()
		levelsIds = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Levels).WhereElementIsNotElementType().ToElementIds()
		filteredElements = FilteredElementCollector(doc, filteredElementsIdsCol).Excluding(camerasIds).Excluding(levelsIds).ToElements()
...
phaseElements = FilteredElementCollector(doc).OfClass(Phase).ToElements()
phaseElementIds = FilteredElementCollector(doc).OfClass(Phase).ToElementIds()
for elId in phaseElementIds:
	phaseEl = doc.GetElement(elId)
	#print("phaseEl name: {0}, Type: {1}".format(phaseEl.Name, phaseEl.GetType()))
#for el in phaseElements:
	#print("Phase el: {0} name: {1}".format(el.Id, el.Name))
