...
class RTD_model(object):
	...
	def setup(self, inDoc, *args, **kwargs):
		...
		#Errors.catchVarErrors.catchVar(self.unwrappedElements,"self.unwrappedElements")
		self.structuredElements = self.getStructuredElements()
		Errors.catchVar(self.structuredElements,"self.structuredElements")
		#self.inserts = ListUtils.flatList(ListUtils.processList(RevitSelection.getInserts, self.getStructuredElements))
		#raise ValueError("self.inserts {0}".format(len(self.inserts), self.inserts))
		#self.filteredInsertsByActiveViewIds = self.filterElementsByActiveViewIds(self.inserts)
		
		""" self.openings = RevitSelection.getOpeningsElements(self.unwrappedElements)
		self.openingFills = RevitSelection.getOpeningsElements(self.unwrappedElements, onlyFills = True)
		#self.openingFillsCategories = ListUtils.processList(self.getCategoryName, self.openingFills)
		self.dynamoOpeningSolids = self.getDynamoSolids(self.openings)
		self.dynamoOpeningFillSolids = self.getDynamoSolids(self.openingFills) """
		...
		Errors.catchVar(self.subtractedOpenings, "self.subtractedOpenings")
		#Errors.catchVar(self.unitedSolidWithOpenings, "self.unitedSolidWithOpenings")
		#self.openingFillSolids = ListUtils.flatList([list(filter(None,[Revit.GeometryConversion.GeometryObjectConverter.ConvertToMany(doc.GetElement(y).Geometry[Autodesk.Revit.DB.Options()]) if doc.GetElement(y).Category.Name != "Walls" else None for y in x.GetDependentElements(None)])) for x in self.openingFills])
		#self.openingFillsGeometry = ListUtils.flattenList([[doc.GetElement(y).Geometry[Autodesk.Revit.DB.Options()] if doc.GetElement(y).Category.Name != "Walls" else None for y in x.GetDependentElements(None)] for x in self.openingFills], Top=True, Level=1)
		#self.openingFillSolids = [x.ToProtoType() for x in self.openingFillsGeometry[0].GetEnumerator()]
		#self.openingFillRevitBoundingBoxes = ListUtils.flatList([list(filter(None,[doc.GetElement(y).Geometry[Autodesk.Revit.DB.Options()].GetBoundingBox() if doc.GetElement(y).Category.Name == "Walls" else None for y in x.GetDependentElements(None)])) for x in self.openingFills])
		#self.openingFillSolidBoundingBoxes = [Autodesk.DesignScript.Geometry.BoundingBox.ByCorners(Autodesk.DesignScript.Geometry.Point.ByCoordinates(x.Min.X, x.Min.Y, x.Min.Z), Autodesk.DesignScript.Geometry.Point.ByCoordinates(x.Max.X, x.Max.Y, x.Max.Z)).ToCuboid() for x in self.openingFillRevitBoundingBoxes]
		#.GetInstanceGeometry() if doc.GetElement(y).Geometry[Autodesk.Revit.DB.Options()].__class__.__name__ == "Autodesk.Revit.DB.GeometryInstance" else None
		#self.pairedOpenings = RevitSelection.findPairsRecursive(self.openingFillSolids, self.subtractedOpenings)
		#Errors.catchVar(self.openingFillSolids, "self.openingFillSolids")
		...
		#self.openings = list(RevitSelection.getElementByClassName(RTD_model.openingsCNs, self.exclude_element_collection))
		#self.windows = list(RevitSelection.getElementByCategory(RTD_model.windowsCgs, self.exclude_element_collection))
		#Errors.catchVar(self.windows, "self.windows")
		#self.doors = list(RevitSelection.getElementByCategory(RTD_model.doorsCgs, self.exclude_element_collection))
		#Errors.catchVar(self.doors, "self.doors")
		#Errors.catchVar(self.openingWalls, "self.openingWalls")
		#Errors.catchVar(self.openingFills, "self.openingFills")

		#get solids from inserts in order to acquire bounding box
		self.openingFillsSolids = ListUtils.flattenList(ListUtils.processList(RevitSelection.getDynamoGeometry, self.openingFills))
		self.openingFillsPairs = zip(self.openingFillsSolids, self.openingFills)
		Errors.catchVar(self.openingFillsPairs, "self.openingFillsPairs")
		#get bounding box from solids of inserts
		self.openingFillsBB = ListUtils.processList(Autodesk.DesignScript.Geometry.BoundingBox.ByGeometry, self.openingFillsSolids)
		self.openingFillsCuboids = ListUtils.processList(Autodesk.DesignScript.Geometry.BoundingBox.ToCuboid, self.openingFillsBB)
		self.openingFillsCentroid = ListUtils.processList(Autodesk.DesignScript.Geometry.Solid.Centroid, self.openingFillsCuboids)
		Errors.catchVar(self.openingFillsBB, "self.openingFillsBB")
		Errors.catchVar(self.openingFillsCuboids, "self.openingFillsCuboids")
		Errors.catchVar(self.openingFillsCentroid, "self.openingFillsCentroid")

		self.openingFillsCentroid_KD =  KD_Tree(self.openingFillsCentroid)
		Errors.catchVar(self.openingFillsCentroid_KD.DSPointsTree, "self.openingFillsCentroid_KD.DSPointsTree")
		
		#Errors.catchVar(self.doorsAndWindowsFills, "self.doorsAndWindowsFills")
		...
		""" self.dynamoSolidsWithOpenings.append(self.dynamoOpeningSolids)
		
		self.unwrappedElements.append(self.openings)
		"""
		#self.categoryIds = ListUtils.processList(self.getCategoryId, self.unwrappedElements)
		#append doors and windows to self.structuredElements
		openingFillsIds = [x.Id for x in self.openingFills]
		openingFillsCol = [Autodesk.Revit.DB.ElementId](openingFillsIds)
		self.doors = DB.FilteredElementCollector(doc, openingFillsCol).OfCategory(Autodesk.Revit.DB.BuiltInCategory.OST_Doors)

		for el in self.openingFills:
			#if el.InternalElement.Category.Name == "Doors":
			self.doors.append(el.Symbol.Family.FamilyCategory.Name)
		Errors.catchVar(self.doors, "self.doors")
		self.structuredElements.append(self.openingFills)
		try:
			self.outerShellIntersectingSurfaces = self.getOuterShellIntersectingSurfaces(self.outerShells, self.dynamoSolids, self.structuredElements)
		...
		Errors.catchVar(self.outerShellIntersectingSurfaces, "self.outerShellIntersectingSurfaces")
		"""
		try:
	...
	def getStructuredElements(self):
		allElementsIds = RevitSelection.getAllElements(toId = True)
			...
			for c, category in enumerate(inSolids):
				...
				for i, item in enumerate(category):
					...
					if  myType == Autodesk.DesignScript.Geometry.Solid and intersectedElements[c][i] == True:
						returnItems.append((item.Intersect(closedOuterShellPolysurface), inRevitElements[c][i]))
...
class EnAnalyse(object):
	...
	def setup(self):
		...
		"""
		self.averageHeatTransferCoefficient = self.getAHTC(self.myQuantities)
		...
		self.facesArea = ListUtils.processList(self.getFacesArea, self.myQuantities)
		"""
	...
	def getAHTC(self, quantities):
		...
		return (objectAreas, [b.Area/1000000 for b in self.outerShells])
