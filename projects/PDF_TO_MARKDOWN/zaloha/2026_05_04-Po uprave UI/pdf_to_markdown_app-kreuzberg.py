...
class PDFToMarkdownApp:
    def __init__(self, root):
        ...
        # Inicializace stavu systému[cite: 5]
        ...
        # --- SEKCE 1: VÝBĚR SOUBORU (Bez rámečku) ---
        ...
        self.select_btn.pack(fill=tk.X, pady=5) # Vyplnění šířky[cite: 5]
        ...
        self.convert_btn.pack(fill=tk.X) # Vyplnění šířky[cite: 5]
        ...
        # Informace o Tesseractu (zobrazena celá cesta)[cite: 5]
        ...
        # Tlačítko pro změnu cesty (vpravo od textu)[cite: 5]
        ...
        # Výběr jazyka (zcela vpravo)[cite: 5]
    ...
    def _find_tessdata_dir(self, tesseract_path):
        ...
        for cand in [base / "tessdata", base / "share" / "tessdata", base.parent / "tessdata"]:
    ...
    def _build_tesseract_info_text(self):
        # Změněno tak, aby vracelo celou cestu[cite: 5]
        t_path = self.tesseract_path if self.tesseract_path else "Nenalezen"
        return f"OCR Engine: {t_path}"
    ...
    def ask_tesseract_path(self):
        path = filedialog.askopenfilename(title="Vyberte tesseract.exe", filetypes=[("Executable", "*.exe"), ("All", "*.*")])
        if path:
    ...
    def _check_ocr_support(self):
        if select_ocr_function is None: return False
        try: return callable(select_ocr_function())
        except: return False
    ...
    def _update_ui_state(self, initial=False):
        ...
        self.convert_btn.config(text="SPUSTIT OCR PDF" if mode == "ocr_pdf" else "PŘEVÉST NA MARKDOWN")
        state = tk.NORMAL if self.selected_file else tk.DISABLED
        if mode == "ocr_pdf":
            for r in self.engine_radios: r.config(state=tk.DISABLED)
            self.ocr_check.config(state=tk.DISABLED)
            if not self.ocr_pdf_available: state = tk.DISABLED
        else:
            for r in self.engine_radios: r.config(state=tk.NORMAL)
    ...
    def convert_file(self):
        ...
        self.status_label.config(text="Zpracovávám dokument...", fg="#FF9800")
    ...
    def _convert_file_thread(self):
        try:
            ...
            if mode == "ocr_pdf":
                ...
                self.root.after(0, self._post_conversion, None, res, None)
            elif self.engine_var.get() == "kreuzberg":
                res = asyncio.run(self._extract_kreuzberg(self.selected_file))
                self.root.after(0, self._post_conversion, res.content, res, None)
            else:
                txt = pymupdf4llm.to_markdown(self.selected_file, use_ocr=self.ocr_enabled_var.get())
                self.root.after(0, self._post_conversion, txt, None, None)
    ...
    def _post_conversion(self, md_text, result, error):
        ...
        if out:
            ...
            else: pathlib.Path(out).write_text(md_text, encoding="utf-8")
            messagebox.showinfo("Hotovo", "Soubor byl úspěšně uložen.")
