...
class InputData():
    ...
    def __init__(self, inPath, inPopData):
        ...
        print(self.confirmedDic["Australia"])
    ...
    def allDataToDic(self, inData):
        ...
        for line in inData:
            ...
            #print("v {}".format([v for i,v in enumerate(line[self.startDataIndex:])]))
            #print("int v {}".format([int(v) if len(v.split(".")) > 1 else int(v.split(".")[0]) if len(v.split(".")) == 1 else 0 for i,v in enumerate(line[self.startDataIndex:])]))
            if line[1] not in dic:
                ...
                newLine = []
                addition = 0
                for i, v in enumerate(line[self.startDataIndex:]):
                    addition += int(v)
                #newLine = [str(int(prevLineData[i]) + int(v) if len(v.split(".")) > 1 else int(v.split(".")[0]) if len(v.split(".")) == 1 else 0 for i,v in enumerate(line[self.startDataIndex:]))]
                #newLine = [str(int(prevLineData[i]) + int(v) for i,v in enumerate(line[self.startDataIndex:]))]
                dic[line[1]] = newLine
        return dic
        
    def getDailyIncreases(self, inDailyData, inName):
        ...
        for i, day in enumerate(inDailyData):
            #print("day {0} dayB {1}".format(day, daysBefore[i]))
...
#            countryDic["active"] = [str(int(x) -int(self.deathsDic[values[0]][j]) - int(self.recoveredDic[values[0]][j])) for j, x in enumerate(values[1])]
            countryDic["increases_Confirmed"] = self.getDailyIncreases(countryDic["confirmed"], values[0])
            countryDic["increases_Deads"] = self.getDailyIncreases(countryDic["deads"], values[0])
