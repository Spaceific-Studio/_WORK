...
#    print "CURRENT RULE: \n" + sourceRule
    slashChar = r"/"
    finalDirPath = inDirPath + "CA2D2r_" + str(int(sourceRule,2)) + "_" + sourceRule + "_" + resXstr + "x" + resYstr + slashChar
...
dirPath = r"/storage/emulated/0/CA/_moje pokusy/CELULAR_AUTOMAT-2D/zaloha/x/"
loadPath = r"/storage/emulated/0/CA/_moje pokusy/CELULAR_AUTOMAT-2D/zaloha/z/"
CSV_logPath = r"/storage/emulated/0/CA/_moje pokusy/CELULAR_AUTOMAT-2D/CSV_log/IMG-sequence/"
writeLogFile = True
...
def createImgCollection(inCA, inDirPath):
    ...
    gap = 60
    headTextFontSize = 13
    headText = "CA2D2_" + str(startPos) + "-" + str(endPos) + "_" + str(resX) + "x" + str(resY) + "x" + str(cols) 
    ruleTextFontSize = 15
...
#    CA = []
    for i, r in enumerate(rules[inRange[0] : inRange[0] + len(inRange)]):
        ...
        run(resolutionX, resolutionY, layersCount, r, dirPath, generateRandom, shiftDir, rndBaseRule)
#        print rules
...
runDirSequence(loadPath, dirPath, range(startRange,endRange))   
