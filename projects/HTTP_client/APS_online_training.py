...
def ziskat_me(token, hub_id, project_id, base_url):
    #url = f"{base_url}/project/v1/hubs/:{hub_id}/projects/:{project_id}/topFolders
    me_url = f"/construction/issues/v1/projects/:b.aeeffa13-fa76-4740-8c75-ccced3afc914/users/me"
    ...
    # Nastavení cesty pro GET požadavek
    conn.request("GET", me_url, headers=headers)
    ...
    response_data = response.read()
    if response.status == 200:
        meData = json.loads(response_data)
        print(f"top folder response: {meData}")
    else:
        print(f"Chyba při získávání me ({response.status}): {response_data}")

    # Uzavřete spojení
    conn.close()
    
ziskat_me(MyCode.getThreeLeggedAuthCode(), hub_id, 'b.aeeffa13-fa76-4740-8c75-ccced3afc914', "https://developer.api.autodesk.com")
    
hubCURLstr = f"https://developer.api.autodesk.com/construction/issues/v1/projects/:b.aeeffa13-fa76-4740-8c75-ccced3afc914/users/me' \
  -H 'Authorization: Bearer {MyCode.getThreeLeggedAuthCode()}'"
try:
    hubResponse = os.popen(hubCURLstr).read()
    print(type(hubResponse))
    print("hubResponse me: {0}".format(hubResponse))
except HTTPError as error:
        print("me {0}".format(sys.exc_info()))
        print(error.status, error.reason)
        
    

hubCURLstr = f"curl -v 'https://developer.api.autodesk.com/project/v1/hubs/:{hub_id}/projects/:b.aeeffa13-fa76-4740-8c75-ccced3afc914/topFolders' \
  -H 'Authorization: Bearer {MyCode.getThreeLeggedAuthCode()}' \
  -H 'x-user-id: gercakd@pvs.cz'"
try:
    hubResponse = os.popen(hubCURLstr).read()
    print(type(hubResponse))
