...
def createNextLayer(inFormerImg, inRules, inInspArray, inLaysCount):
    ...
    if writeLogFile == True:
        f = open(CSV_logPath + "CA2D9_" + myHexCAstr[2:len(myHexCAstr)-1] + "_" + str(liWidth) + "x" + str(liHeight) + ".csv", 'w')
        writer = csv.writer(f)
...
#    print "inMyRule in run {0}".format(inMyRule)
#    print "rule in run hex {:x}".format(int(sourceRule,2))
...
groupOneUnwantedSeq = [0,1,8,9,16,17,24,25,18,19,26,27,29]
...
dirPath = r"C:/_WORK/PYTHON/CELULAR_AUTOMAT-2D/RNDCA2D9-sequence/"
...
def createImgCollection(inCA, inDirPath):
    ...
    myCArule = myCAdata[2]
    print "myCArule in createImgCollection{}".format(myCArule)
    myCA = myCAdata[0]
    img0 = myCA[0]
    resX = img0.size[0]
    resY = img0.size[1]
    ...
    headText = "CA2D9_" + myHeadRule[2:len(myHeadRule)-1] + "_" + str(resX) + "x" + str(resY) + "x" + str(cols) 
    ruleTextFontSize = 10
...
#    IDfieldWidth = ruleTextXsize + 200
    ...
    print "myBRule in createImgCollection".format(myBRule)
    myHR = str(hex(int(myBRule, 2)))
    myHRule = myHR[2:len(myHR)-1]
    lReadPath = CSV_logPath + "CA2D9_" + myHRule + "_" + str(resX) + "x" + str(resY) + ".csv"
...
#        drawCanvas.text((0,yPos + (1*ruleTextYsize*0.7)), "POSITION: " + str(ca[1]),font=headTextFont)
    ...
    print "file was saved >> {0}".format(fullPath)
    print "resolution X: {0}, resolution Y: {1}, columns: {2}, rows: {3}".format(resX, resY, cols, rows)
...
def runSequence():
    ...
    myInspArray = getInspArray(9)
    mySymOnesSequence = getInspArraySeqence(len(groupOneSym))
    mySymOnesCleanedSequence = []
    for m, h in enumerate(mySymOnesSequence):
        if not m in groupOneUnwantedSeq:
            mySymOnesCleanedSequence.append(h)
#    mySymOnesSequence.pop(groupOneUnwantedSeq)
#    print "mySymOnesCleanedSequence"
#    print mySymOnesCleanedSequence
    for i,s in enumerate(mySymOnesCleanedSequence):
        symOnePos = []
        #rules.append(generateRandomRule(myInspArray))
        rules.append(getInspArrayRangeZero(9))
        rules[i] = filterRule(rules[i],sym4axis + sym2axis)
        rules[i] = forceSymetryGroups(rules[i], myRndSymGroups)
        for j,v in enumerate(s):
            if v == 1:
                symOnePos += groupOneSym[j]
        rules[i] = forceSymetryGroups(rules[i], symOnePos)
        position = int(rules[i],2)
#        print "order num - {0}".format(num)
#        print "symOnePos:"
#       print symOnePos
#        print "mySymOnesCleanedSequence[" + str(i) +"]:"
#        print mySymOnesCleanedSequence[i]
#        print "position " + str(position)
        #print "hexrule in runSequence {:x}".format(int(rules[i],2))
        ...
        thread = SyncRun(i, resolutionX, resolutionY, layersCount, rules[i], dirPath, generateRandom, shiftDir, rndBaseRule)
        threads.append(thread)
        threads[i].start()        
        num += 1
    print "Number of threads - {0}".format(len(threads))
    for t in threads:
        print "Joining {0}".format(t.name)
        t.join()
    for i,t in enumerate(threads):
        CA.append([t.CA, position, rules[i]])
    createImgCollection(CA, dirPath)
    overalETime = time.time()
    ovTime = overalETime - overalSTime
    print "overal time of generating " + str(len(mySymOnesCleanedSequence)) + "CAs is " + str(int(ovTime/60)) + "min " + str(ovTime%60)+ " seconds"


class SyncRun(threading.Thread):
    def __init__(self, inId, inResX, inResY, inLayCount, inMyRule, inDirPath, inGenerateRandom, inShiftDir, inRndBaseRule):
        threading.Thread.__init__(self)
        self.iD = inId
        self.resX = inResX
        self.resY = inResY
        self.layCount = inLayCount
        self.myRule = inMyRule
        self.dirPath = inDirPath
        self.generateRandom = inGenerateRandom
        self.shiftDir = inShiftDir
        self.rndBaseRule = inRndBaseRule
        self.CA = None
        
        
    def run(self):
        sTime = time.ctime(time.time())
        print "Start time of CA_thread-{0} is {1}".format(self.iD, time.ctime(time.time()))
        # Get lock to synchronize threads 
        threadLock.acquire()
        self.CA = run(self.resX, self.resY, self.layCount, self.myRule, self.dirPath, self.generateRandom, self.shiftDir, self.rndBaseRule)
        # Free lock to release next thread 
        threadLock.release()
        print "Generation of CA-{0} thread done".format(self.iD)
        print "Start time {1}".format(self.iD, sTime)
        print "End   time {1}".format(self.iD, time.ctime(time.time()))
...
def runRndSequence(inGroupeSettings):
    ...
    for i in range(0, endRange):
        ...
        rules[i] = filterRule(rules[i],sym4axis + sym2axis)
        rules[i] = forceSymetryGroups(rules[i], myRndSymGroups)
        myRndSymOnes = getRndSymetryGroups([], groupOneSym)
        rules[i] = forceSymetryGroups(rules[i], myRndSymOnes[0])
        position = int(rules[i],2)
        print num
        print "position " + str(position)
        thread = SyncRun(i, resolutionX, resolutionY, layersCount, rules[i], dirPath, generateRandom, shiftDir, rndBaseRule)
...
#myThread = SyncRun(1, resolutionX, resolutionY, layersCount, "".join(myRule), dirPath, generateRandom, shiftDir, rndBaseRule)
#myThread.start()
runSequence()
